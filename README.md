...
| Full Stack Development/Engineering | Well adept in implementing UI and UX designs using numerous tech stacks which include but are not limited to MERN, MEAN, and LAMP Stack. Also adept in utiliziing widget frameworks which include but are not limited to Qt C++ and Tkinter. Also adept in modular programming to breed opportunities for reusing components to solve other technical problems.  |
